...
function ChatView({ socket, accessToken }) {
  ...
  const [cussor, setCussor] = useState()
  ...
  // let socket = socketio('http://localhost:8080/chat', {
  // let socket = socketio('http://localhost:8080', {
  //   transports: ['websocket'],
  // auth: { token: localStorage.getItem('login-token') }
  // })

  // 밖에 있으면 토큰 전달이 되고, useEffect안에 있으면 최초1회 제외하고 소켓아이디가 유지됨
  // 토큰 전달도 되면서 소켓아이디 유지도 되야함
  ...
  return (
    <div>
      ...
      <div className='chat-body-container'>
        ...
        <ChatList roomList={roomList} joinRoom={joinRoom} />
